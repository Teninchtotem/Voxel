...
namespace Win
{
    ...
    public:
		...
		bool checkHandleIsValid(HWND in){ return IsWindow(in); }
    ...
    public:
        ...
        ~TextBox() {}
		void setText(const wchar_t* buf) 
		{ 
			if (checkHandleIsValid(handle) && checkHandleIsValid(parent))
			{ 
				SendMessage(handle, WM_SETTEXT, 0, (LPARAM)buf); 
			} 
		}
