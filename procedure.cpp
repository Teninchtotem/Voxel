...
INT_PTR CALLBACK Win::dialogProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		ctrl->keyDown(LOWORD(wParam), HIWORD(wParam));
		return true;

	case WM_KEYUP:
		ctrl->keyUp(LOWORD(wParam), HIWORD(wParam));
		return true;
	...
	case WM_MOUSEWHEEL:
		ctrl->mouseWheel((short)LOWORD(wParam), (short)HIWORD(wParam) / WHEEL_DELTA, (short)LOWORD(lParam), (short)HIWORD(lParam));   // state, delta, x, y
		return true;
